...
// Inertia is the main Fibernetia structure, which contains all the logic for being an Inertia adapter.
type Inertia struct {
...
type FlashProvider interface {
	Flash(c *fiber.Ctx, key string, val any) error
	Get(c *fiber.Ctx, key string) (any, error)
	GetAll(c *fiber.Ctx) (map[string]any, error)
	ShouldClearHistory(c *fiber.Ctx) (bool, error)
	FlashClearHistory(c *fiber.Ctx) error
}
